...
mod tests {
    ...
    #[test]
    fn inet_filter_4tcp_bare() {
        let mut f = empty_filter();
        parse_inet_filter("4TCP", &mut f);
        assert_eq!(f.network_type, Some(4));
        assert_eq!(f.network_filters[0].protocol.as_deref(), Some("TCP"));
    }
    ...
    #[test]
    fn inet_filter_4udp_port() {
        let mut f = empty_filter();
        parse_inet_filter("4UDP:53", &mut f);
        assert_eq!(f.network_type, Some(4));
        let nf = &f.network_filters[0];
        assert_eq!(nf.protocol.as_deref(), Some("UDP"));
        assert_eq!(nf.port_start, Some(53));
    ...
    fn inet_filter_6tcp() {
        let mut f = empty_filter();
        parse_inet_filter("6TCP", &mut f);
        assert_eq!(f.network_type, Some(6));
        assert_eq!(f.network_filters[0].protocol.as_deref(), Some("TCP"));
    }

    #[test]
    fn inet_filter_6tcp_port() {
        let mut f = empty_filter();
    ...
    #[test]
    fn inet_filter_6udp_port() {
        let mut f = empty_filter();
        parse_inet_filter("6UDP:53", &mut f);
        assert_eq!(f.network_type, Some(6));
        let nf = &f.network_filters[0];
        assert_eq!(nf.protocol.as_deref(), Some("UDP"));
        assert_eq!(nf.port_start, Some(53));
    ...
    #[test]
    fn from_args_inet_udp_at_ipv4_host_with_port() {
        let args = Args::parse_from(["lsofrs", "-i", "UDP@192.168.0.1:123"]);
        let f = Filter::from_args(&args);
        assert!(f.network);
        assert!(f.network_filters.iter().any(|nf| {
            nf.protocol.as_deref() == Some("UDP")
                && nf.host.as_deref() == Some("192.168.0.1")
                && nf.port_start == Some(123)
        }));
    }
    ...
    #[test]
    fn from_args_inet_tcp_at_test_net_host_port() {
        let args = Args::parse_from(["lsofrs", "-i", "TCP@192.0.2.1:443"]);
        let f = Filter::from_args(&args);
        assert!(f.network);
        assert!(f.network_filters.iter().any(|nf| {
    ...
    fn from_args_inet_tcp_at_ipv4_host_port() {
        ...
        assert!(f.network_filters.iter().any(|nf| {
            nf.protocol.as_deref() == Some("TCP")
                && nf.host.as_deref() == Some("10.0.0.5")
                && nf.port_start == Some(443)
        }));
    }
    ...
    #[test]
    fn from_args_inet_udp_at_ipv4_host_no_port() {
        let args = Args::parse_from(["lsofrs", "-i", "UDP@10.0.0.9"]);
        let f = Filter::from_args(&args);
        assert!(f.network);
        assert!(f.network_filters.iter().any(|nf| {
            nf.protocol.as_deref() == Some("UDP")
    ...
    #[test]
    fn from_args_inet_tcp_port_53() {
        let args = Args::parse_from(["lsofrs", "-i", "TCP:53"]);
        let f = Filter::from_args(&args);
        assert!(f.network);
        assert_eq!(f.network_filters[0].protocol.as_deref(), Some("TCP"));
    ...
    #[test]
    fn from_args_inet_tcp_port_22() {
        let args = Args::parse_from(["lsofrs", "-i", "TCP:22"]);
        let f = Filter::from_args(&args);
        assert!(f.network);
        assert_eq!(f.network_filters[0].protocol.as_deref(), Some("TCP"));
        assert_eq!(f.network_filters[0].port_start, Some(22));
    }

    #[test]
    fn from_args_inet_udp_port_22() {
        let args = Args::parse_from(["lsofrs", "-i", "UDP:22"]);
        let f = Filter::from_args(&args);
        assert!(f.network);
        assert_eq!(f.network_filters[0].protocol.as_deref(), Some("UDP"));
        assert_eq!(f.network_filters[0].port_start, Some(22));
    ...
    #[test]
    fn from_args_inet_tcp_port_2() {
        let args = Args::parse_from(["lsofrs", "-i", "TCP:2"]);
        let f = Filter::from_args(&args);
        assert!(f.network);
        assert_eq!(f.network_filters[0].protocol.as_deref(), Some("TCP"));
        assert_eq!(f.network_filters[0].port_start, Some(2));
    }

    #[test]
    fn from_args_inet_tcp_port_3() {
        let args = Args::parse_from(["lsofrs", "-i", "TCP:3"]);
        let f = Filter::from_args(&args);
        assert!(f.network);
        assert_eq!(f.network_filters[0].protocol.as_deref(), Some("TCP"));
        assert_eq!(f.network_filters[0].port_start, Some(3));
    }
    ...
    #[test]
    fn from_args_inet_udp_port_2() {
        let args = Args::parse_from(["lsofrs", "-i", "UDP:2"]);
        let f = Filter::from_args(&args);
        assert!(f.network);
        assert_eq!(f.network_filters[0].protocol.as_deref(), Some("UDP"));
        assert_eq!(f.network_filters[0].port_start, Some(2));
    ...
    #[test]
    fn from_args_inet_udp_port_3() {
        let args = Args::parse_from(["lsofrs", "-i", "UDP:3"]);
        let f = Filter::from_args(&args);
        assert!(f.network);
        assert_eq!(f.network_filters[0].protocol.as_deref(), Some("UDP"));
        assert_eq!(f.network_filters[0].port_start, Some(3));
    ...
    #[test]
    fn from_args_inet_udp_port_80() {
        let args = Args::parse_from(["lsofrs", "-i", "UDP:80"]);
        let f = Filter::from_args(&args);
        assert!(f.network);
        assert_eq!(f.network_filters[0].protocol.as_deref(), Some("UDP"));
    ...
    #[test]
    fn from_args_inet_udp_port_65535() {
        let args = Args::parse_from(["lsofrs", "-i", "UDP:65535"]);
        let f = Filter::from_args(&args);
        assert!(f.network);
        assert_eq!(f.network_filters[0].protocol.as_deref(), Some("UDP"));
    ...
    #[test]
    fn from_args_inet_udp_port_67() {
        let args = Args::parse_from(["lsofrs", "-i", "UDP:67"]);
        let f = Filter::from_args(&args);
        assert!(f.network);
        assert_eq!(f.network_filters[0].protocol.as_deref(), Some("UDP"));
        assert_eq!(f.network_filters[0].port_start, Some(67));
    }
    ...
    #[test]
    fn from_args_inet_4tcp_port_53() {
        let args = Args::parse_from(["lsofrs", "-i", "4TCP:53"]);
        let f = Filter::from_args(&args);
        assert!(f.network);
        assert_eq!(f.network_type, Some(4));
        assert!(
    ...
    #[test]
    fn from_args_inet_6tcp_with_port() {
        let args = Args::parse_from(["lsofrs", "-i", "6TCP:443"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(6));
        assert!(f.network);
        assert!(
    ...
    #[test]
    fn from_args_inet_6tcp_port_80() {
        let args = Args::parse_from(["lsofrs", "-i", "6TCP:80"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(6));
        assert!(f.network);
        assert!(
    ...
    #[test]
    fn from_args_inet_6tcp_port_22() {
        let args = Args::parse_from(["lsofrs", "-i", "6TCP:22"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(6));
        assert!(f.network);
        assert!(
    ...
    #[test]
    fn from_args_inet_6tcp_port_1() {
        let args = Args::parse_from(["lsofrs", "-i", "6TCP:1"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(6));
        assert!(f.network);
        assert!(
            f.network_filters
                .iter()
                .any(|nf| { nf.protocol.as_deref() == Some("TCP") && nf.port_start == Some(1) })
        );
    }

    #[test]
    fn from_args_inet_6tcp_port_2() {
        let args = Args::parse_from(["lsofrs", "-i", "6TCP:2"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(6));
        assert!(f.network);
        assert!(
            f.network_filters
                .iter()
                .any(|nf| { nf.protocol.as_deref() == Some("TCP") && nf.port_start == Some(2) })
        );
    }

    #[test]
    fn from_args_inet_6tcp_port_3() {
        let args = Args::parse_from(["lsofrs", "-i", "6TCP:3"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(6));
        assert!(f.network);
        assert!(
            f.network_filters
                .iter()
                .any(|nf| { nf.protocol.as_deref() == Some("TCP") && nf.port_start == Some(3) })
        );
    }
    ...
    #[test]
    fn from_args_inet_6tcp_port_65535() {
        let args = Args::parse_from(["lsofrs", "-i", "6TCP:65535"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(6));
        assert!(f.network);
        assert!(
            f.network_filters.iter().any(|nf| {
                nf.protocol.as_deref() == Some("TCP") && nf.port_start == Some(65535)
            })
        );
    }
    ...
    #[test]
    fn from_args_inet_6udp_port_80() {
        let args = Args::parse_from(["lsofrs", "-i", "6UDP:80"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(6));
        assert!(f.network);
        assert!(
    ...
    #[test]
    fn from_args_inet_6udp_port_1() {
        let args = Args::parse_from(["lsofrs", "-i", "6UDP:1"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(6));
        assert!(f.network);
        assert!(
            f.network_filters
                .iter()
                .any(|nf| { nf.protocol.as_deref() == Some("UDP") && nf.port_start == Some(1) })
        );
    }

    #[test]
    fn from_args_inet_6udp_port_2() {
        let args = Args::parse_from(["lsofrs", "-i", "6UDP:2"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(6));
        assert!(f.network);
        assert!(
            f.network_filters
                .iter()
                .any(|nf| { nf.protocol.as_deref() == Some("UDP") && nf.port_start == Some(2) })
        );
    }

    #[test]
    fn from_args_inet_6udp_port_3() {
        let args = Args::parse_from(["lsofrs", "-i", "6UDP:3"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(6));
        assert!(f.network);
        assert!(
            f.network_filters
                .iter()
                .any(|nf| { nf.protocol.as_deref() == Some("UDP") && nf.port_start == Some(3) })
        );
    }

    #[test]
    fn from_args_inet_6udp_port_22() {
        let args = Args::parse_from(["lsofrs", "-i", "6UDP:22"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(6));
        assert!(f.network);
        assert!(
            f.network_filters
                .iter()
                .any(|nf| { nf.protocol.as_deref() == Some("UDP") && nf.port_start == Some(22) })
        );
    }

    #[test]
    fn from_args_inet_6udp_port_65535() {
        let args = Args::parse_from(["lsofrs", "-i", "6UDP:65535"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(6));
        assert!(f.network);
        assert!(
            f.network_filters.iter().any(|nf| {
                nf.protocol.as_deref() == Some("UDP") && nf.port_start == Some(65535)
            })
        );
    }
    ...
    #[test]
    fn from_args_inet_tcp_bracket_ipv6_host_no_port() {
        let args = Args::parse_from(["lsofrs", "-i", "TCP[::1]"]);
        let f = Filter::from_args(&args);
        assert!(f.network);
        assert!(f.network_filters.iter().any(|nf| {
    ...
    #[test]
    fn from_args_inet_udp_bracket_ipv6_host_no_port() {
        let args = Args::parse_from(["lsofrs", "-i", "UDP[::1]"]);
        let f = Filter::from_args(&args);
        assert!(f.network);
        assert!(f.network_filters.iter().any(|nf| {
    ...
    #[test]
    fn from_args_inet_tcp_bracket_ipv6_host_port() {
        let args = Args::parse_from(["lsofrs", "-i", "TCP[::1]:443"]);
        let f = Filter::from_args(&args);
        assert!(f.network);
        assert!(f.network_filters.iter().any(|nf| {
            nf.protocol.as_deref() == Some("TCP")
    ...
    #[test]
    fn from_args_inet_4udp_port_443() {
        let args = Args::parse_from(["lsofrs", "-i", "4UDP:443"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(4));
        assert!(f.network);
        assert!(
    ...
    #[test]
    fn from_args_inet_4udp_port_67() {
        let args = Args::parse_from(["lsofrs", "-i", "4UDP:67"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(4));
        assert!(f.network);
        assert!(
    ...
    #[test]
    fn from_args_inet_4udp_port_1() {
        let args = Args::parse_from(["lsofrs", "-i", "4UDP:1"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(4));
        assert!(f.network);
        assert!(
    ...
    #[test]
    fn from_args_inet_4udp_port_2() {
        let args = Args::parse_from(["lsofrs", "-i", "4UDP:2"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(4));
        assert!(f.network);
        assert!(
    ...
    #[test]
    fn from_args_inet_4udp_port_3() {
        let args = Args::parse_from(["lsofrs", "-i", "4UDP:3"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(4));
        assert!(f.network);
        assert!(
    ...
    #[test]
    fn from_args_inet_4udp_port_22() {
        let args = Args::parse_from(["lsofrs", "-i", "4UDP:22"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(4));
        assert!(f.network);
        assert!(
    ...
    #[test]
    fn from_args_inet_4udp_port_123() {
        let args = Args::parse_from(["lsofrs", "-i", "4UDP:123"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(4));
        assert!(f.network);
        assert!(
    ...
    #[test]
    fn from_args_inet_4udp_port_65535() {
        let args = Args::parse_from(["lsofrs", "-i", "4UDP:65535"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(4));
        assert!(f.network);
        assert!(
            f.network_filters
                .iter()
                .any(|nf| nf.protocol.as_deref() == Some("UDP") && nf.port_start == Some(65535))
        );
    }
    ...
    #[test]
    fn from_args_inet_4tcp_port_443() {
        let args = Args::parse_from(["lsofrs", "-i", "4TCP:443"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(4));
        assert!(f.network);
        assert!(
    ...
    #[test]
    fn from_args_inet_4tcp_port_67() {
        let args = Args::parse_from(["lsofrs", "-i", "4TCP:67"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(4));
        assert!(f.network);
        assert!(
    ...
    #[test]
    fn from_args_inet_4tcp_port_1() {
        let args = Args::parse_from(["lsofrs", "-i", "4TCP:1"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(4));
        assert!(f.network);
        assert!(
    ...
    #[test]
    fn from_args_inet_4tcp_port_2() {
        let args = Args::parse_from(["lsofrs", "-i", "4TCP:2"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(4));
        assert!(f.network);
        assert!(
    ...
    #[test]
    fn from_args_inet_4tcp_port_3() {
        let args = Args::parse_from(["lsofrs", "-i", "4TCP:3"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(4));
        assert!(f.network);
        assert!(
    ...
    #[test]
    fn from_args_inet_4tcp_port_123() {
        let args = Args::parse_from(["lsofrs", "-i", "4TCP:123"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(4));
        assert!(f.network);
        assert!(
    ...
    #[test]
    fn from_args_inet_4tcp_port_65535() {
        let args = Args::parse_from(["lsofrs", "-i", "4TCP:65535"]);
        let f = Filter::from_args(&args);
        assert_eq!(f.network_type, Some(4));
        assert!(f.network);
        assert!(
            f.network_filters
                .iter()
                .any(|nf| nf.protocol.as_deref() == Some("TCP") && nf.port_start == Some(65535))
        );
    }
