...
Dispatch order matters when multiple output modes are set: for example `--stale`, `--ports`, and `--pipe-chain` run before `--csv`; `--csv` runs before `--net-map`, `--tree`, and `--summary` (and before default `--json` output); `--json` runs before `-t` terse, which runs before `-F` field output. Integration tests cover combinations such as `-J` with `-F`, `--csv` with `-J`, `-J` with `-t`, `--csv` with `-t`, `--stale`/`--ports`/`--pipe-chain` versus `--csv`, `--csv` versus `--net-map`, `--net-map` versus `--tree`, `--tree` versus `--summary`, `-t` with `-F`, `--stale` with `-t`, `--tree`/`--summary`/`--net-map`/`--ports`/`--pipe-chain` with `-J` and `-t`, and `--csv` with `--summary` or `--tree` so the winning mode stays stable.
